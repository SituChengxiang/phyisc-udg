...
- 极坐标变换：$x=r\cos\theta, y=r\sin\theta$，在区域$D$是圆、环、扇形或者其一部分的时候比较常用$$dA = r dr d\theta$$
...
- 球坐标变换：球体、锥体：$x=\rho\sin\phi\cos\theta, y=\rho\sin\phi\sin\theta, z=\rho\cos\phi$，则$dV = \rho^2 \sin\phi d\rho d\phi d\theta$

1. 确定积分区域$\Omega$
2. 投影到面：常用投影到  $xy$  平面（先  $z$  后  $xy$）、$xz$  平面（先  $y$  后  $xz$）或  $yz$  平面（先  $x$  后  $yz$）。选择依据是边界曲面表示哪种方式最简单。然后体现不出来的那个坐标成为第一个积分序
3. 确定积分上下限： 
...
6. 化为累次积分计算：例如投影到  $xy$  平面：$$\iiint_{\Omega} f(x, y, z) dV = \iint_{D}\left[ \int_{z_1(x,y)}^{z_2(x,y)} f(x, y, z) dz\right] dA$$其中  $\iint_D ... dA$  是对投影区域  $D_{xy}$  的二重积分。
...
1. 计算弧长微元$ds$：$$ds=\left|r'(t)\right|=\sqrt{\left(\frac{dx}{dt}\right)^2+\left(\frac{dy}{dt}\right)^2+\left(\frac{dz}{dt}\right)^2}$$可以看出来$ds$必然是正的，如果表示为$ds=\sqrt{1+\left(\dfrac{dy}{dx}\right)^2}dx$，则$dx$也必然为正，方向是定死的；对弧长积分下限一定小于上
2. 带入$x(t),y(t),z(t)$
